...
Enhanced with:
- Lever-specific button vocabulary
...
LEVER_APPLY_WORDS = ["apply for this job", "apply now", "apply", "submit application"]
LEVER_SUBMIT_WORDS = ["submit application", "submit", "send application", "apply now"]
LEVER_NEXT_WORDS = ["continue", "next", "review"]
LEVER_EEO_WORDS = ["eeo", "equal employment", "voluntary self-identification", 
...
class LeverAdapter(GenericATSAdapter):
    ...
    def plan(self, req: StepRequest, profile: Profile) -> Optional[list[Action]]:
        ...
        # 1) Check for "Apply for this job" reveal button (some Lever boards)
        apply_btn = find_button(els, LEVER_APPLY_WORDS)
        if apply_btn:
    ...
    def _find_resume_upload(self, els) -> Optional[object]:
        ...
        for e in els:
            if e.tag == "input" and e.type == "file":
                label = text_of(e)
                if any(w in label for w in ["resume", "cv", "upload", "attach", "file", "dropzone"]):
                    return e
        return None

    def _handle_eeo_questions(self, els, profile: Profile) -> Optional[list[Action]]:
        """Handle Lever's EEO/demographic questions (usually optional)."""
        for e in els:
            label = text_of(e).lower()
            if any(w in label for w in LEVER_EEO_WORDS):
    ...
    def _looks_done(self, els) -> bool:
        ...
        done_markers = [
            ...
            "confirmation", "application complete", "you're all set"
        ]
        for e in els:
            t = text_of(e).lower()
